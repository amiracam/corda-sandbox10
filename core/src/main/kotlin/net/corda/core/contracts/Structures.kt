...
// Anything that can be used in building a Transaction from TransactionBuilder
// todo: @amiracam
interface TransactionBuildable {
    fun addToBuilder(builder: TransactionBuilder)
}
 ...
 */
@CordaSerializable
interface ContractState : TransactionBuildable {
    /**
    ...
    override fun addToBuilder(builder: TransactionBuilder) {
        builder.addOutputState(this)
    }
}
...
data class TransactionState<out T : ContractState> @JvmOverloads constructor(
         ...
         */
        val encumbrance: Int? = null) : TransactionBuildable {
    override fun addToBuilder(builder: TransactionBuilder) {
        //todo: amiracam
        builder.addOutputState(this)
    }
...
@CordaSerializable
data class StateAndRef<out T : ContractState>(val state: TransactionState<T>, val ref: StateRef) : TransactionBuildable {
    override fun addToBuilder(builder: TransactionBuilder) {
        builder.addInputState(this)
    }
}
...
@CordaSerializable
interface CommandData : TransactionBuildable {

    override
    fun addToBuilder(builder: TransactionBuilder) {
        throw IllegalArgumentException("You passed an instance of CommandData, but that lacks the pubkey. You need to wrap it in a Command object first.")
    }
...
@CordaSerializable
data class Command(val value: CommandData, val signers: List<CompositeKey>) : TransactionBuildable {
    //todo: amiracam
    override
    fun addToBuilder(builder: TransactionBuilder) {
